...
class CartController extends Controller
{
    ...
    public function destroy(Request $request, Product $product, CartService $cartService)
    {
        ...
        return back()->with('success', 'Item was removed from cart!');
    }
    ...
    public function checkOut(Request $request, CartService $cartService)
    {
        ...
        $address = $request->user()->addresses()->where('primary', true)->first()
            ?? $request->user()->addresses()->first();

        if (! $address) {
            return back()->with('error', 'Please add an address before checkout.');
        }
        ...
        try {
            ...
            Log::error($e);
            DB::rollBack();
            return back()->with('error', $e->getMessage() ?: 'Soemthing went wrong!');
        }
    }
}
